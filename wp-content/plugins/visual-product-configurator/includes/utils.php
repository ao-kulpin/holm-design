...
function o_admin_fields( $options ) {
	...
	foreach ( $options as $value ) {
		...
		if ( ! in_array( $value['type'], $section_types, true ) & ! empty( $hierarchy ) ) {
			...
			if ( ! $option_value ) {
				...
				if ( 'metas' === $settings_table ) {
					...
					$option_value                       = get_proper_value( $options[0], 'data', array() );
					$_SESSION['o-data'][ $session_key ] = $option_value;
				}
			}
			...
			if ( $root_key !== $value['name'] ) {
				$option_value = find_in_array_by_key( $root_value, $value['name'] );
			}
		}
		$col_class = get_proper_value( $value, 'col_class', '' );
		...
		switch ( $value['type'] ) {
			...
			case 'textarea':
				?>
				<textarea name="<?php echo esc_attr( $value['name'] ); ?>" id="<?php echo esc_attr( $value['id'] );?>" style="<?php echo esc_attr( $value['css'] );?>" class="<?php echo esc_attr( $value['class'] ); ?>" <?php echo esc_attr( implode( ' ', $custom_attributes ) ); ?>><?php echo $option_value;?></textarea>
				<?php
				break;
				...
				?>
				   ...
				   <?php
					foreach ( $value['options'] as $key => $val ) {
						$option_custom_attributes = get_proper_value( $options_custom_attributes, $key, array() );
						?>
							<option
			...
			case 'image':
				...
				if ( $root_img_src ) {
					$img_src = get_medias_root_url( "/$root_img_src" );
				}
				?>
			...
			case 'repeatable-fields':
				...
				$lazy_mode      = get_proper_value( $value, 'lazyload', false );
				...
				if ( $lazy_mode ) {
					$spinner_gif_url = get_proper_value( $value, 'spinner_gif', false );
					echo wp_kses_post(
						sprintf(
				...
				foreach ( $value['fields'] as $field ) {
					...
					$col_class = get_proper_value( $field, 'col_class', '' );
			...
			case 'groupedfields':
				...
				foreach ( $value['fields'] as $field ) {
					...
					echo wp_kses( o_admin_fields( array( $field ) ), get_allowed_tags() );
				}
				?>
					</div>
			...
			case 'button':
				...
				break;
			case 'google-font':
				get_google_fonts_selector( $option_value, esc_attr( $value['id'] ), esc_attr( $value['name'] ), esc_attr( $value['css'] ), esc_attr( $value['class'] ) );
				break;
...
function get_row_template( $value ) {
	$row_class = get_proper_value( $value, 'row_class', '' );
 ...
 * @return false|mixed
 */
function find_in_array_by_key( $root_value, $key ) {
...
function o_get_proper_image_url( $suspected_link, $with_root = true ) {
	...
	if ( is_numeric( $suspected_link ) ) {
		...
		$img_src     = str_replace( get_medias_root_url( '/' ), '', $raw_img_src );
	}
	$img_src = str_replace( get_medias_root_url( '/' ), '', $img_src );
	// Code for bad https handling.
	if ( strpos( get_medias_root_url( '/' ), 'https' ) === false ) {
		$https_home = str_replace( 'http', 'https', get_medias_root_url( '/' ) );
		$img_src    = str_replace( $https_home, '', $img_src );
	}

	if ( $with_root ) {
		$img_src = get_medias_root_url( "/$img_src" );
	}
 ...
 * @return mixed
 */
function get_google_fonts_selector( $selected_font = false, $id = '', $name = '', $style = '', $class = '' ) {
 ...
 * @return string|void
 */
function get_medias_root_url( $path = '/' ) {
 ...
 * @return void
 */
function register_google_font( $font_name, $raw_url ) {
