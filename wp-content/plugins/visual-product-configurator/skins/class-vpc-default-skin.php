...
class VPC_Default_Skin {
	...
	<div id="vpc-container" class="o-wrap <?php echo esc_attr( $skin_name ); ?>" data-curr="<?php echo ( class_exists( 'Woocommerce' ) ) ? esc_attr( get_woocommerce_currency_symbol() ) : ''; ?>">
		...
		?>
		<div class="o-col conf_desc"><?php echo wp_kses_post( $conf_desc ); ?></div>
		...
		echo wp_kses( $preview_html, get_allowed_tags() );
	...
	public function get_components_block( $component, $options_style, $config, $config_to_load = array() ) {
		...
		if ( isset( $c_img_url ) && ! empty( $c_img_url ) ) {
	...
	<div id = '<?php echo esc_attr( $component_id ); ?>' class="vpc-component" <?php echo esc_attr( $components_attributes_string ); ?>>
		...
		?>
		<span class="vpc-selected txt"><?php esc_html_e( 'none', 'vpc' ); ?></span></span>
		...
		if ( ! is_array( $options ) || empty( $options ) ) {
			esc_html_e( 'No option detected for the component. You need at least one option per component for the configuration to work properly.', 'vpc' );
		} else {
			...
			foreach ( $options as $option_index => $option ) {
				...
				switch ( $component['behaviour'] ) {
					...
					case 'checkbox':
						...
						if ( ! empty( $tooltip ) ) {
							$tooltip = "data-oriontip='" . esc_html( $tooltip ) . "'";
						}
						?>
